...
# Try to import Autumna scraper for pricing data
try:
    from api_clients.autumna_scraper import AutumnaScraper
    AUTUMNA_AVAILABLE = True
except ImportError:
    AUTUMNA_AVAILABLE = False
    logger.warning("AutumnaScraper not available, using static pricing data")
...
class NegotiationStrategyService:
    ...
    async def generate_negotiation_strategy(
        ...
        # Try to fetch Autumna pricing data
        autumna_data = None
        if postcode:
            try:
                # Extract city/location from postcode for Autumna search
                location = postcode.split(' ')[0] if postcode else None
                if location:
                    autumna_data = await self._fetch_autumna_pricing(
                        location=location,
                        care_type=care_type,
                        proxy_url=autumna_proxy_url
                    )
            except Exception as e:
                logger.warning(f"Failed to fetch Autumna pricing: {str(e)}")
    ...
    async def _fetch_autumna_pricing(
        ...
        """Fetch pricing data from Autumna if available"""
        if not AUTUMNA_AVAILABLE:
            return None
    ...
    def _generate_market_rate_analysis(
        ...
        # Use Autumna data if available, otherwise use static averages
        if autumna_data:
            logger.info(f"Using Autumna pricing data: avg={autumna_data.get('avg_price')}, sample_size={autumna_data.get('sample_size')}")
            # Adjust regional average based on Autumna data if available
            autumna_avg = autumna_data.get('avg_price')
            if autumna_avg is not None:
                try:
                    autumna_avg = float(autumna_avg)
                    # Blend Autumna data with regional average (weighted 70% Autumna, 30% regional)
                    regional_average = (autumna_avg * 0.7) + (regional_average * 0.3)
                except (ValueError, TypeError):
                    pass  # Keep regional_average as is
        ...
        # Add Autumna data if available
        if autumna_data:
            market_analysis['autumna_data'] = {
                'source': autumna_data.get('source'),
