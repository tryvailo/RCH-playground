...
class ProfessionalMatchingService:
    ...
    def _calculate_financial_stability(
        ...
        - Working capital: 6 points
        - Bankruptcy risk: 6 points
        """
        score = 0.0
        max_score = 20.0
        ...
        # 1. Profitability (8 points)
        profitability_score = 0.0
        net_margin = financial_data.get('net_margin_3year_avg')
        ...
        if current_ratio is not None:
            try:
                ...
                if current_ratio >= 2.0:
                    working_capital_score = 6.0
                elif current_ratio >= 1.5:
                    working_capital_score = 4.0
                elif current_ratio >= 1.0:
                    working_capital_score = 2.0
                # < 1.0 = 0
            except (ValueError, TypeError):
                pass
        ...
        # 3. Bankruptcy risk - Altman Z-score (6 points)
        bankruptcy_score = 6.0
        altman_z = financial_data.get('altman_z_score')
        if altman_z is not None:
            try:
                altman_z = float(altman_z)
                if altman_z < 1.8:  # High risk
                    bankruptcy_score = 0.0
                elif altman_z < 2.5:  # Medium risk
                    bankruptcy_score = 2.0
                elif altman_z < 3.0:  # Low risk
