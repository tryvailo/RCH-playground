...
/**
 * Loads XLS file and converts to ArrayBuffer
 */
async function fetchXLSFile(url: string): Promise<ArrayBuffer> {
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(`Failed to fetch MSIF file: ${response.statusText}`);
  }
...
async function parseMSIFXLS(arrayBuffer: ArrayBuffer): Promise<MSIFData> {
  try {
    ...
    if (Object.keys(msifData).length > 0) {
      console.log(`✅ Parsed MSIF data for ${Object.keys(msifData).length} local authorities`);
      return msifData;
    }
    ...
    console.warn('Failed to parse MSIF XLS file, using fallback data:', error);
...
async function loadMSIFData(): Promise<MSIFData> {
  ...
  if (cached) {
    try {
      ...
      if (age < CACHE_EXPIRY_MS) {
        console.log('✅ MSIF data loaded from cache');
        return data;
      }
    } catch (e) {
      console.warn('Failed to parse cached MSIF data', e);
    }
  }
  ...
  if (store.msifData && store.lastFetched) {
    ...
    if (age < CACHE_EXPIRY_MS) {
      console.log('✅ MSIF data loaded from Zustand store');
  ...
  try {
    ...
    try {
      console.log('📥 Loading MSIF 2025-2026...');
      ...
      console.warn('⚠️ MSIF 2025-2026 failed, trying 2024-2025...', error);
    ...
    useMSIFStore.getState().setError(errorMessage);
    console.error('❌ Failed to load MSIF data:', error);
...
export async function getFairCostLower(
  ...
  try {
    ...
    if (!laData) {
      console.warn(`⚠️ MSIF data not found for ${normalizedLA}, trying fallback`);
      ...
      if (similarLA) {
        ...
        return fallbackData[careType] || null;
      }
      return null;
    }
    
    return laData[careType] || null;
  } catch (error) {
    console.error('Error getting Fair Cost Lower:', error);
    return null;
  }
}
