...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const hasCredentials = credsResponse.data;

      // Mock API status (in production, this would come from backend)
      const apiList: ApiStatusInfo[] = [
        { name: 'CQC', status: hasCredentials.has_cqc ? 'connected' : 'disconnected', successRate: 95, avgResponseTime: 0.8 },
        { name: 'FSA', status: 'connected', successRate: 98, avgResponseTime: 0.5 },
        { name: 'Companies House', status: hasCredentials.has_companies_house ? 'connected' : 'disconnected', successRate: 92, avgResponseTime: 1.2 },
        { name: 'Google Places', status: hasCredentials.has_google_places ? 'connected' : 'disconnected', successRate: 90, avgResponseTime: 1.5 },
        { name: 'Perplexity', status: hasCredentials.has_perplexity ? 'connected' : 'disconnected', successRate: 88, avgResponseTime: 3.0 },
        { name: 'BestTime', status: hasCredentials.has_besttime ? 'connected' : 'disconnected', successRate: 75, avgResponseTime: 5.0 },
        { name: 'Autumna', status: hasCredentials.has_autumna ? 'connected' : 'disconnected', successRate: 70, avgResponseTime: 8.0 },
      ];
