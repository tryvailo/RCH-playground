...
- Custom Care Home Financial Risk Model (SPEC v3.2) - NOT Altman Z-Score!
- Director history & stability analysis
- Financial stability scoring (0-100)
- Risk assessment with signals

Based on spec v3.2 PROFESSIONAL Report requirements:
- Financial Stability uses Custom Care Home Financial Risk Model
- 5 components: Liquidity (30%), Debt (25%), Profitability (25%), Management (10%), Maturity (10%)
- ⚠️ Altman Z-Score NOT suitable for care homes (asset-heavy, low current ratios normal)
"""
...
class CompaniesHouseService:
    ...
    async def get_financial_stability(
        ...
        try:
            ...
            # Extract financial data for risk model
            financial_data = self._extract_financial_data_for_risk_model(
                profile, charges, company_age, director_stability
            )
            ...
            # Convert risk score (0-100, lower = safer) to algorithm points (0-20)
            # Invert: 0 risk = 20 points, 100 risk = 0 points
            algorithm_score = max(0, 20 - int(risk_result.risk_score / 5))
            
            # Keep Altman Z-Score for backward compatibility (but mark as deprecated)
            altman_z = self._calculate_altman_z_proxy(
                profile, officers, charges, company_age
            )
            altman_z.methodology_note = (
                "⚠️ DEPRECATED: Altman Z-Score not suitable for care homes. "
                "Using Custom Care Home Financial Risk Model instead."
            )
            
        except ImportError:
            # Fallback to old method if new service not available
            logger.warning("CareHomeFinancialRiskService not available, using Altman Z-Score proxy")
            altman_z = self._calculate_altman_z_proxy(
                profile, officers, charges, company_age
            )
            algorithm_score = (
    ...
    def _extract_financial_data_for_risk_model(
        self,
        profile: Dict,
        charges: List[Dict],
        company_age: float,
        ...
        """
        Extract financial data for Custom Care Home Financial Risk Model
        
        Since Companies House API doesn't provide full financial statements,
        we use estimates based on available data:
        - Charges for debt estimation
        - Accounts status for profitability inference
        - Company age for maturity
        - Director stability for management
        
        Args:
            profile: Company profile from Companies House
            charges: List of charges (debt indicators)
            company_age: Company age in years
            director_stability: Director stability analysis
            
        Returns:
            Dict with financial data for risk model
        """
        # Estimate current assets and liabilities based on charges
        # Care homes typically have property assets, so estimate conservatively
        outstanding_charges = [c for c in charges if not c.get('satisfied_on')]
        total_charges_value = sum(
            float(c.get('amount_secured', 0) or 0) for c in outstanding_charges
        )
        
        # Estimate current assets (conservative for care homes)
        # Typical care home: £500K-2M in assets depending on size
        # Use charges as proxy for debt, estimate assets at 2-3x debt
        if total_charges_value > 0:
            estimated_total_assets = total_charges_value * 2.5
            estimated_current_assets = estimated_total_assets * 0.15  # ~15% current assets
        else:
            # No charges - estimate based on company age
            if company_age >= 10:
                estimated_current_assets = 500000  # Established company
            elif company_age >= 5:
                estimated_current_assets = 300000
            else:
                estimated_current_assets = 150000
        
        # Estimate current liabilities
        # Typical care home: current liabilities ~20-30% of current assets
        estimated_current_liabilities = estimated_current_assets * 0.25
        
        # Estimate total debt from charges
        estimated_total_debt = total_charges_value if total_charges_value > 0 else estimated_current_assets * 0.4
        
        # Estimate equity (assets - liabilities)
        estimated_equity = estimated_total_assets - estimated_total_debt - estimated_current_liabilities
        if estimated_equity < 0:
            estimated_equity = estimated_total_assets * 0.3  # Minimum 30% equity
        
        # Estimate profit/loss based on accounts status
        accounts = profile.get('accounts', {})
        accounts_overdue = accounts.get('overdue', False)
        last_accounts = accounts.get('last_accounts', {}).get('made_up_to')
        
        # Infer profit trend from accounts status
        if accounts_overdue:
            estimated_profit = -50000  # Loss if overdue
            ...
            try:
                ...
                days_since = (date.today() - last_date).days
                if days_since < 400:
                    # Recent accounts - assume profitable
                    estimated_profit = estimated_total_assets * 0.05  # 5% return
                    profit_trend = 'stable'
                else:
                    estimated_profit = estimated_total_assets * 0.02  # Lower return
                    profit_trend = 'declining'
            except:
                estimated_profit = estimated_total_assets * 0.03
                profit_trend = 'unknown'
        else:
            estimated_profit = estimated_total_assets * 0.03
            profit_trend = 'unknown'
        
        # Estimate depreciation (typical for care homes: 2-3% of assets)
        estimated_depreciation = estimated_total_assets * 0.025
        
        # Director changes in 3 years
        director_changes_3yr = director_stability.resignations_last_2_years  # Approximate
        ...
        return {
            ...
            'company_age_years': company_age,
            'data_quality': 'estimated',  # Mark as estimated
            'notes': 'Financial data estimated from Companies House indicators (charges, accounts status, company age)'
        }
