...
function CareHomeCard({ home, index }: { home: CareHomeData; index: number }) {
  // Calculate average price with proper null checks
  const avgPrice = home.price_range && home.price_range.min != null && home.price_range.max != null
  ...
  if (avgPrice === 0) {
    console.warn(`⚠️ Price display issue for ${home.name}:`, {
      price_range: home.price_range,
      has_price_range: !!home.price_range,
      min: home.price_range?.min,
      max: home.price_range?.max
    });
  }
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-all duration-300">
      ...
      <div className="p-6">
        ...
        <div className="mb-4 pb-4 border-b border-gray-200">
          <div className="flex items-baseline justify-between">
            <div>
              ...
              {avgPrice > 0 ? (
                ...
                <p className="text-3xl font-bold text-gray-400">Price not available</p>
              )}
            </div>
            {home.fsa_color && (
              <div 
          ...
          {home.price_range && home.price_range.min != null && home.price_range.max != null ? (
            <p className="text-xs text-gray-500 mt-1">
              £{home.price_range.min.toLocaleString()} - £{home.price_range.max.toLocaleString()}/week
            </p>
          ) : (
            <p className="text-xs text-gray-400 mt-1">Price range not available</p>
          )}
        </div>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between items-center">
            ...
            <span className="font-medium text-gray-900">{(home.distance ?? 0).toFixed(1)} km</span>
          </div>
          {home.rating && (
            <div className="flex justify-between items-center">
              <span className="text-gray-600 flex items-center">
  ...
  if (!homes || homes.length < 3) {
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <p className="text-gray-600">Not enough homes for comparison (need at least 3 homes)</p>
      </div>
    );
  }

  // Calculate average weekly costs
  const avgCost1 = ((homes[0]?.price_range?.min ?? 0) + (homes[0]?.price_range?.max ?? 0)) / 2;
  const avgCost2 = ((homes[1]?.price_range?.min ?? 0) + (homes[1]?.price_range?.max ?? 0)) / 2;
  const avgCost3 = ((homes[2]?.price_range?.min ?? 0) + (homes[2]?.price_range?.max ?? 0)) / 2;

  const criteria = [
    {
      name: 'Weekly Cost',
      home1: `£${avgCost1.toLocaleString()}`,
      home2: `£${avgCost2.toLocaleString()}`,
      home3: `£${avgCost3.toLocaleString()}`,
    },
    {
      name: 'Price Band',
    ...
    {
      name: 'Distance',
      home1: `${(homes[0]?.distance ?? 0).toFixed(1)} km`,
      home2: `${(homes[1]?.distance ?? 0).toFixed(1)} km`,
      home3: `${(homes[2]?.distance ?? 0).toFixed(1)} km`,
    },
    {
      name: 'FSA Rating',
...
export default function ReportRenderer({ report, questionnaire }: ReportRendererProps) {
  ...
  if (!report.homes || report.homes.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <p className="text-gray-600">No care homes found in the report</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-12">
      ...
      <div>
        ...
        {homes && homes.length > 0 ? (
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <p className="text-gray-600 text-center">No care homes found matching your criteria.</p>
          </div>
        )}
      </div>
      ...
      {questionnaire && report.fairCostGap && (() => {
        ...
        if (fcg && fcg.weekly > 0 && fcg.annual > 0) {
          const msifLower = fcg.msifLower || 700;
