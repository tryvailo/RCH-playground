...
import { OSPlacesResults, ONSResults, OSMResults, NHSBSAResults, EnvironmentalResults, MapView } from './components';
...
export default function NeighbourhoodExplorer() {
  ...
  const [formData, setFormData] = useState({
    ...
    include_osm: true,
    include_nhsbsa: true,
    include_environmental: false, // Disabled by default until stable
  });
  ...
  // Extract coordinates and address info for map
  const mapData = useMemo(() => {
    ...
    let postcode: string | undefined = result.postcode;
    ...
    // Priority 2: OS Places centroid
    else if (result.os_places?.centroid?.latitude && result.os_places?.centroid?.longitude) {
      lat = result.os_places.centroid.latitude;
      lon = result.os_places.centroid.longitude;
    }
    // Priority 3: First OS Places address
    else if (result.os_places?.addresses?.[0]?.latitude && result.os_places?.addresses?.[0]?.longitude) {
      lat = result.os_places.addresses[0].latitude;
      lon = result.os_places.addresses[0].longitude;
    }
    // Priority 4: ONS geography
    else if (result.ons?.geography?.latitude && result.ons?.geography?.longitude) {
      lat = result.ons.geography.latitude;
      lon = result.ons.geography.longitude;
    }

    // Get address from OS Places
    if (result.os_places?.addresses?.[0]) {
      const firstAddr = result.os_places.addresses[0];
      address = firstAddr.address || `${firstAddr.building_name || ''} ${firstAddr.street || ''}`.trim() || undefined;
    }

    // Add markers for all OS Places addresses if multiple
    if (result.os_places?.addresses && result.os_places.addresses.length > 1) {
      result.os_places.addresses.forEach((addr, idx) => {
        if (addr.latitude && addr.longitude) {
          markers.push({
            lat: addr.latitude,
            lon: addr.longitude,
            label: addr.address || `Address ${idx + 1}`
          });
        }
      });
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <SourceToggle
                checked={formData.include_environmental}
                onChange={(checked) => setFormData({ ...formData, include_environmental: checked })}
                icon={<Activity className="w-5 h-5 text-orange-600" />}
                label="Environmental"
                description="Noise & Pollution (Beta)"
                color="orange"
              />
            </div>
          </div>
      ...
      {result && (
        <div className="space-y-4">
            ...
            >
              <div className="space-y-4">
                <MapView
                  ...
                  zoom={mapData.markers && mapData.markers.length > 1 ? 13 : 15}
                  height="500px"
                  showMultipleMarkers={mapData.markers}
                />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    {mapData.postcode && (
                      <div>
                        ...
                        <div className="text-gray-600 font-mono mt-1">{mapData.postcode}</div>
                      </div>
                    )}
                    {mapData.address && (
                      <div className="md:col-span-2">
                        <span className="font-medium text-gray-700">Address:</span>
                        <div className="text-gray-600 mt-1">{mapData.address}</div>
                      </div>
                    )}
                    {mapData.markers && mapData.markers.length > 1 && (
                      <div className="md:col-span-2">
                        <span className="font-medium text-gray-700">Multiple addresses found:</span>
                        <div className="text-gray-600 mt-1">{mapData.markers.length} locations shown on map</div>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {result.environmental && (
            ...
            >
              <EnvironmentalResults data={result.environmental} />
            </CollapsibleSection>
          )}
