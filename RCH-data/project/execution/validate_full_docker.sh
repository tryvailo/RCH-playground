...
# 5.1: Индексы (исправлено: фактически 49 индексов - 47 явных + 2 автоматических)
...
check_result "$([ "$INDEX_COUNT" -ge "49" ] && echo "0" || echo "1")" "Индексов: $INDEX_COUNT (ожидается 49-53: 47 явных + 2 автоматических + возможные дополнительные)" ""

# GIN индекс на regulated_activities (исправлено: ищем по имени индекса или по типу)
GIN_EXISTS=$(docker exec cqc-validation-db psql -U postgres -d cqc_validation -t -c "SELECT COUNT(*) FROM pg_indexes WHERE tablename='care_homes' AND (indexname LIKE '%regulated_activities%' OR (indexdef LIKE '%regulated_activities%' AND indexdef LIKE '%USING gin%'));" | tr -d ' ')
...
# Проверка структуры JSONB (исправлено: ищем jsonb_build_object с activities в любом виде)
if grep -q "jsonb_build_object" "$SQL_MIGRATION" && grep -A 5 "jsonb_build_object" "$SQL_MIGRATION" | grep -q "activities"; then
...
for view in "${VIEWS[@]}"; do
    # Исправлено: проверяем и через information_schema, и через pg_views
    EXISTS=$(docker exec cqc-validation-db psql -U postgres -d cqc_validation -t -c "
        SELECT COUNT(*) FROM information_schema.views 
        WHERE table_schema='public' AND table_name='$view';
    " 2>/dev/null | tr -d ' ' || echo "0")
    
    # Если не найдено через information_schema, проверяем через pg_views
    if [ "$EXISTS" = "0" ]; then
        EXISTS=$(docker exec cqc-validation-db psql -U postgres -d cqc_validation -t -c "
