...
ORDER BY 
    CASE
        ...
        WHEN (latitude IS NULL OR longitude IS NULL) THEN 3
        WHEN (latitude = 0 OR longitude = 0) THEN 2
        WHEN (year_registered > EXTRACT(YEAR FROM CURRENT_DATE)) THEN 1
        WHEN (cqc_rating_overall IS NULL AND (care_nursing = TRUE OR care_residential = TRUE)) THEN 3
        WHEN (care_dementia = TRUE AND serves_dementia_band = FALSE) THEN 4
        WHEN (regulated_activities = '{"activities": []}'::jsonb AND has_nursing_care_license = TRUE) THEN 4
        WHEN (data_quality_score < 50) THEN 2
        WHEN (is_dormant = TRUE AND updated_at > CURRENT_TIMESTAMP - INTERVAL '30 days') THEN 5
        ELSE 5
    END,
