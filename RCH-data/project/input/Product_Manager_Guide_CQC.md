...
### Ошибка #6: Неправильные имена полей Service User Bands

**Симптомы:**
- Ошибки при загрузке CSV: `column "service_user_band_children_0_17_years" does not exist`
- Данные не загружаются для новых полей v2.2
- NULL значения в полях `serves_children`, `serves_detained_mha`, `serves_substance_misuse`

**Причина:**
Имена полей в SQL не соответствуют реальным именам колонок в CSV файле CQC.

**Неправильные имена (исправлено):**
```sql
-- ❌ НЕПРАВИЛЬНО (было)
service_user_band_children_0_17_years          -- должно быть 0_18_years
service_user_band_detained_under_the_mental_health_act  -- должно быть people_detained_...
service_user_band_people_misusing_drugs_and_alcohol     -- должно быть people_who_misuse...
service_user_band_learning_disabilities_or_autistic_spectrum_d  -- должно быть _di вместо _d
```
...
**Проверка:**
```sql
-- Убедитесь, что поля существуют в CSV
SELECT column_name FROM information_schema.columns 
WHERE table_name = 'temp_cqc_raw' 
AND column_name LIKE 'service_user_band%';
```

### Ошибка #7: Использование несуществующего поля для care_respite

**Симптомы:**
- Ошибка: `column "service_type_respite" does not exist`
- Ошибка при выполнении миграции

**Причина:**
В CQC CSV файле нет поля `service_type_respite`, но оно использовалось в маппинге.
...
### Ошибка #8: Поля в INSERT не соответствуют схеме БД

**Симптомы:**
- Ошибка: `column "registered_manager" does not exist`
- Ошибка: `column "address_line_1" does not exist`
- Ошибка при выполнении INSERT

**Причина:**
В INSERT SELECT были включены поля, которых нет в схеме БД v2.2.

**Решение:**
Удалить следующие поля из INSERT SELECT:
- `registered_manager` - нет в схеме БД v2.2
- `address_line_1` - нет в схеме БД v2.2
- `address_line_2` - нет в схеме БД v2.2

**Проверка:**
```sql
-- Убедитесь, что все поля в INSERT существуют в схеме
SELECT column_name FROM information_schema.columns 
...
## ✅ ИСПРАВЛЕНИЯ МАППИНГА (27 января 2025)

### Исправленные ошибки в именах полей:

1. **`service_user_band_children_0_17_years` → `service_user_band_children_0_18_years`**
   - ✅ Исправлено: было `0_17_years`, стало `0_18_years`
   - ✅ Соответствует реальному CSV файлу

2. **`service_user_band_detained_under_the_mental_health_act` → `service_user_band_people_detained_under_the_mental_health_act`**
   - ✅ Исправлено: добавлен префикс `people_`
   - ✅ Соответствует реальному CSV файлу

3. **`service_user_band_people_misusing_drugs_and_alcohol` → `service_user_band_people_who_misuse_drugs_and_alcohol`**
   - ✅ Исправлено: было `people_misusing`, стало `people_who_misuse`
   - ✅ Соответствует реальному CSV файлу

4. **`service_user_band_learning_disabilities_or_autistic_spectrum_d` → `service_user_band_learning_disabilities_or_autistic_spectrum_di`**
   - ✅ Исправлено: добавлен `i` в конце (`_di` вместо `_d`)
   - ✅ Соответствует реальному CSV файлу

5. **`care_respite` маппинг исправлен:**
   - ✅ Было: `safe_boolean(service_type_respite)` (предположение)
   - ✅ Стало: `NULL` (нет в CQC CSV, согласно документации)

6. **Удалены поля из INSERT (нет в схеме БД v2.2):**
   - ✅ `registered_manager` - удалено
   - ✅ `address_line_1` - удалено  
   - ✅ `address_line_2` - удалено

**Результат:** Все 93 поля теперь правильно маппятся (100% соответствие)
