...
try:
    ...
    from scoring_constants import ScoringRatios, DefaultScores, ScoringBaseMax, ScoringDefaults
...
class MatchingService:
    ...
    def __init__(
        ...
        Args:
            scoring_weights: Dict with weights for each category (location, cqc, budget, careType, availability, googleReviews)
            scoring_thresholds: Dict with thresholds for location, budget, googleReviews scoring
        """
        # Default weights (total = 100) - use constants
        self.weights = scoring_weights or ScoringDefaults.DEFAULT_WEIGHTS.copy()
        
        # Default thresholds - use constants
        self.thresholds = scoring_thresholds or ScoringDefaults.DEFAULT_THRESHOLDS.copy()
        
        # Normalize weights to ensure they sum to 100 (if custom weights provided)
        if scoring_weights:
            total = sum(self.weights.values())
            if total != 100 and total > 0:
                # Scale weights proportionally
                for key in self.weights:
                    self.weights[key] = int((self.weights[key] / total) * 100)
    ...
    def score_cqc_rating(self, rating: Optional[str]) -> int:
        """
        Calculate CQC rating score (uses configured weight)
